...
// Fonction utilitaire pour détecter le type de ressource Cloudinary
const getResourceType = (file) => {
  ...
  if (['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp', 'tiff', 'svg'].includes(extension) ||
      mimeType.startsWith('image/')) {
    return 'image';
  }
  
  // Par défaut, utiliser 'auto' pour les autres types
  return 'auto';
};
  ...
  try {
    ...
    if (Array.isArray(document.fichiers) && document.fichiers.length > 0) {
      for (const file of document.fichiers) {
        ...
        try {
          // Déterminer le resource_type pour la suppression
